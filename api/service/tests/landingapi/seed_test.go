...
func insertSeedData(db *dbtest.Database) (seedData, error) {
	...
	landing, err := db.BusDomain.Landing.Save(ctx, landingbus.NewLanding{
		Label: label.MustParse("Seed Landing"),
	})
	if err != nil {
		return seedData{}, fmt.Errorf("seeding landing: %w", err)
	}
	...
	return seedData{
		...
		Landings: []landingbus.Landing{landing},
	}, nil
}
