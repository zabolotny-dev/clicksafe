...
var fromHTTPStatus = map[int]ErrCode{
	http.StatusBadRequest:            InvalidArgument,
	http.StatusUnauthorized:          Unauthenticated,
	http.StatusForbidden:             PermissionDenied,
	http.StatusNotFound:              NotFound,
	http.StatusMethodNotAllowed:      NotFound,
	http.StatusConflict:              AlreadyExists,
	http.StatusRequestEntityTooLarge: ResourceExhausted,
...
func FromHTTPStatus(status int) ErrCode {
	if code, ok := fromHTTPStatus[status]; ok {
		return code
	}
	return Internal
}
